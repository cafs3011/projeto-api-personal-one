  ...
  let aluno = await AlunoRepository.criar(request.body);
    ...
    if (aluno) {
      if (
        ...
        request.body.dataFim
      ) {
        let personal = await PersonalRepository.buscarPorUsuario(
          request.body.usuario_id
        );
        let assinaturaJson = {
        ...
        let assinatura = await AssinaturaRepository.criar(
          assinaturaJson,
          "assinaturaModel"
        );
        let alunoRetorno = {
          ...
          dataFimAssinatura: assinatura.dataFim
        };
        response.status(status.CREATED).send(alunoRetorno);
      } else response.status(status.CREATED).send(aluno);
    } else response.status(status.INTERNAL_SERVER_ERROR).send();
  } catch (error) {
    next(error);
  }
};
